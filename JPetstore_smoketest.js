...
export const options = {vus:1,iteration:1}
// export const options = {
//   stages: [
//     { duration: '300s', target: 100 }, // Ramp-up to 100 users in 5 minutes
//     { duration: '3100s', target: 100 }, // Stay at 100 users for 55 minutes
...
var p_pet = csvDataParam[Math.floor(Math.random() * csvDataParam.length)]['Values']
...
export default function()
{
  ...
  let c_sourcePage02;
  let c_orderId;
  let randomumber = Math.floor(Math.random() * 900000000000000) + 100000000000000;
      // Convert to string and insert spaces for formatting
  let cardNumber = randomumber.toString().replace(/(\d{3})(\d{4})(\d{4})(\d{4})/, '$1 $2 $3 $4')
  ...
  let n =2;
    ...
    group('T01_Launch',function t01_launch (){
        ...
        check(response,{
            ...
            "checking assertion launch: ": (r) => r.body.includes('Welcome to JPetStore 6'),
        });
        sleep(n);
    })
  ...
  //<a href="/actions/Account.action;jsessionid=C68917EF8226A73836802FA7ACC469EE?signonForm=
          ...
          c_jSession_id1 = findBetween(response.body, 'a href="/actions/Account.action;jsessionid=','?signonForm=')
      ...
      // https://petstore.octoperf.com/actions/Account.action;jsessionid=C31686B0ED28B56483097F9905C2308A?signonForm=
        ...
        let header = {headers: {
          ...
          check(response2,{
            ...
            "checking Click signin enterStore: ": (r) => r.body.includes('Please enter your username and password.'),
          ...
          corr_sourcePage01 = response2.html().find('input[name=_sourcePage]').attr('value');
...
//              'F0CFdf--zTwyK88v9T3SsShSO6XoXHVrA8VkB_SnWEqqxs9V0e4UHmwvXtWoYpxZ7PKV3dleW7K-j9I4A6vkyKCTfSCwxVI5ySd1aEesaVg=',
      ...
      group('T05_Select_Pet_category', function t04_selectPet(){
        ...
        let regex1 = /<a href="\/actions\/Catalog\.action\?viewProduct=&amp;productId=(.*?)">/g
        ...
        check(response,{
          "checking T05_Select_Pet_category response code: " : (r1) => r1.status===200,
          "checking T05_Select_Pet_category enterStore: ": (r1) => r1.body.includes(`${corr_prod}`),
      ...
      group('T06-Pet_SelectItem', function(){
        ...
        let regexp2 = /<a href="\/actions\/Catalog\.action\?viewItem=&amp;itemId=(.*?)">/g
        let itemID =[]
        ...
        check(res,{
          ...
          "checking T06_Pet_SelectItem enterStore: ": (r2) => r2.body.includes(`${corr_itemid}`),
        })
        sleep(n);
      })
      ...
      group('T07_ViewItem', function(){
        ...
        check(response,{
          ...
          "checking T07_viewItem enterStore: ": (r2) => r2.body.includes(`${corr_itemid}`),
        })
        sleep(n);
      })
      ...
      group('T08-Click Add to cart', function(){
        ...
        check(response,{
          "checking T08_Click Add to cart response code: " : (r2) => r2.status===200,
          "checking T08_Click Add to cart enterStore: ": (r2) => r2.body.includes('Proceed to Checkout'),
        });
        sleep(n);
      })
      ...
      group('T09_Proceed_to_checkout', function(){
        ...
        let url = 'https://petstore.octoperf.com/actions/Order.action?newOrderForm='
        let head2 = {headers: {
          'content-type': 'application/x-www-form-urlencoded',
          origin: 'https://petstore.octoperf.com',
          'upgrade-insecure-requests': '1',
        ...
        let fp=[]
        let sourcepage = []
        let spregexp3 = /\<input type="hidden" name="_sourcePage" value="(.*?)" \/>/g
        ...
        check(response,{
          "checking T09_Proceed_to_checkout response code: " : (r3) => r3.status===200,
          "checking T09_Proceed_to_checkout response text: " : (r3) => r3.body.includes('Payment Details'),
        });
        sleep(n);
      })
      ...
      group('T10_EnterDetails_clickonContinue', function(){
        ...
        console.log("random3: "+ random3 + " zipCode: "+ zipCode + " Card Number : "+ cardNumber)
      ...
      console.log("T10_EnterDetails_clickonContinue response status code: "+ response.status)
      check(response,{
        "Checking T10_EnterDetails_clickonContinue status code: ": (r4) => r4.status===200,
        "Checking T10_EnterDetails_clickonContinue text check: ": (r4) => r4.body.includes('confirm'),
      ...
      group('T11_ClickonConfirm', function(){
        let url = 'https://petstore.octoperf.com/actions/Order.action?newOrder=&confirmed=true'
        let headd = {headers: {
          'content-type': 'application/x-www-form-urlencoded',
          origin: 'https://petstore.octoperf.com',
          'upgrade-insecure-requests': '1',
        ...
        check(cresponse,{
          ...
          "Checking T11_ClickonCofirm text check: ": (r5) => r5.body.includes('Thank you, your order has been submitted.'),
      ...
      group('T12_Signout', function(){
        ...
        let responses = http.batch([['GET', url1, null, head7], // API 1 (signoff)
                                 ['GET', url2, null, head8],]) // API 2 (catalog) 
...
export function handleSummary(data) {
  return {
    "pet_smoketest2_oct23_2024.html": htmlReport(data),
